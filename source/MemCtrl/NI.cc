...
{
#ifdef _DEBUG
    _DBG_NOARG(" handle_new_packet_event ");
#endif
        HighLevelPacket* hlp;
        if(e->src_id == interface_connections[0]->address)
            ...
            hlp = outstanding_hlp;
    if (!((MC*)mc)->reqH->oneBufferFull && hlp)
    {
	Request* req = new Request();
	convertFromBitStream(req, hlp);
    	req->arrivalTime = Simulator::Now(); //hlp->recv_time-missed_time;	
        total_missed_time += (Simulator::Now() - hlp->recv_time);
        packets++;
        last_pkt_out_cycle = Simulator::Now();
...
/* 
    else
    {
        missed_time++;
	IrisEvent* e2 = new IrisEvent();
        e2->type = NEW_PACKET_EVENT;
	e2->event_data.push_back(hlp);
        Simulator::Schedule(Simulator::Now()+1, &NetworkComponent::process_event, this, e2);
    }
*/
...
NI::handle_out_pull_event(IrisEvent* e)
{
    ...
    if (ready[0] && GetFromNIQueue(req))
    {
	    ...
	    convertToBitStream(req, hlp);      
            ...
            total_backward_time += (Simulator::Now() - req->retireTime);
            packets_out++;
...
NI::print_stats() const
{
    ...
    str << "NI [" << node_ip << "] " << "packets: " << packets << endl;
    str << "NI [" << node_ip << "] " << "packets_out: " << packets_out << endl;
    str << "NI [" << node_ip << "] " << "total_backward_time: " << total_backward_time << endl;
    str << "NI [" << node_ip << "] " << "Avg Latency waiting in NI (fwd path): " << (total_missed_time+0.0)/packets << endl;
    str << "NI [" << node_ip << "] " << "Avg Latency waiting in NI (bwd path): " << (total_backward_time+0.0)/packets_out << endl;
