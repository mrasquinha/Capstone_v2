...
GenericRouterVct::print_stats()
{
    ...
    str << "\n router[" << node_ip << "] packets: " << packets
        ...
        << "\n router[" << node_ip << "] Simulation Rate microsecs/packet: " 
        << (stat_sim_total_time+0.0)/packets
...
GenericRouterVct::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    if(data->type == FLIT_ID)
    {
        ...
        if( data->ptr->type == HEAD )
        {
            ...
            input_buffer_state[port*vcs+data->vc].stat_pkt_intime = tv.tv_usec;
...
GenericRouterVct::do_switch_traversal()
{
    ...
    _DBG(" Tail FO clear pkt for inport %d oport %d ", iport, oport);
                        ...
                        struct timeval tv;
                        gettimeofday(&tv,0);
                        stat_sim_total_time += (tv.tv_usec - input_buffer_state[i].stat_pkt_intime);
                    }
                    else
                    {
#ifdef _DEBUG_ROUTER
