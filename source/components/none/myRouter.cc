...
#ifndef  _myrouter_cc_INC
#define  _myrouter_cc_INC
...
MyRouter::MyRouter ()
{
    name = "Router" ;
    ticking = false;
}  /* -----  end of method MyRouter::MyRouter  (constructor)  ----- */
...
void
MyRouter::set_no_ports( uint p )
{
    ports = p;
}

void
MyRouter::set_no_vcs( uint v )
{
    vcs = v;
}

void
MyRouter::set_no_credits( int c)
{
    credits = c;
}

void
MyRouter::set_buffer_size( uint b)
{
    buffer_size = b;
}
...
void
MyRouter::set_grid_x_location( uint port, uint x_node, uint value)
{
    decoders[port].grid_xloc[x_node]= value;
}

void
MyRouter::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
MyRouter::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        ...
        case TICK_EVENT:
            ...
            break;
        default:
            _DBG("MyRouter:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function MyRouter::process_event  ----- */

string
MyRouter::print_stats()
{
    stringstream str;
...
void
MyRouter::handle_link_arrival_event ( IrisEvent* e )
{
...
//        data->valid = false;
...
#ifdef _DEBUG_ROUTER
        _DBG(" BW/Decoded inport:%d invc:%d oport:%d ovc:%d ft:%d length:%d", port, data->vc,
              mstate[port*vcs+data->vc].output_port, mstate[port*vcs+data->vc].output_channel, data->ptr->type,mstate[port*vcs+data->vc].credits_sent);
#endif
        }
        else
        {
    ...
    return ;
}		/* -----  end of function MyRouter::handle_link_arrival_event  ----- */

void
MyRouter::handle_tick_event ( IrisEvent* e )
{
...
}		/* -----  end of function MyRouter::handle_input_arbitration_event  ----- */

string
MyRouter::toString() const
{
    stringstream str;
    str << "MyRouter"
        << "\t addr: " << address
...
void
MyRouter::send_credit_back(uint i)
{
